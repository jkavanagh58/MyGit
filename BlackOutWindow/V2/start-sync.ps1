<#
	...
	.NOTES
		...
		06.22.2018 JJK: ------------------------ Starting V2 Development ------------------------
		06.22.2018 JJK:	TODO: Change Write-LogEntry to Write-LogEntry
		07.06.2018 JJK:	File moved from production will be used for Version 2 source
		07.07.2018 JJK:	Removed functions from script and placed in mmodule
#>
[CmdletBinding()]
Param(
...
PROCESS {
	ForEach ($ccmColl in $ccmcollections | sort-object -Property Name ){
		...
		Write-LogEntry -logmessage "Collected Instances $($Instances.count)" -LogFile $logFile -entryType "Information"
