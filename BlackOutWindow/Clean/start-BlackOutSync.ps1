<# Comment Based Help
	...
	.NOTES
		...
		06.21.2017 JJK: TODO: Error handling and logging
...
Param(
	$SiteServer = 'SCCMSiteServerName',
	$ManagementServer = "FQDNSCOMMgmtServer",
...
BEGIN {
	Function clear-scomgroup {
		Param(
			[String]$ManagementServer = "SCOMMgmtServer",
			[String]$GroupName,
			$ManagementPackID,
			$GroupID
		)
	...
	Start-OperationsManagerClientShell -ManagementServerName: "FQDNSCOMMgmtServer" -PersistConnection: $true -Interactive: $true
	...
	$ccmcollections = get-wmiobject @collQuery  | sort-object -Property Name
}
PROCESS {
	ForEach ($ccmColl in $ccmcollections){
		...
		If ($Instances.Count -ge 1){
			try {
				...
				# Write record to autumation database
	...
	# Execute processes
	& $callReport
	#"Updating Scheduled task {0}" -f $SCOMGroup
	#Invoke-Expression -Command $processtasks
} # end of Process
END {
