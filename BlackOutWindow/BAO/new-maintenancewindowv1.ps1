...
BEGIN {
	If (($StartStopMM -eq "Start") -and ($MMTime -eq $null)){
		Write-Error -Message "You must enter a number of minutes for this process"
		Exit
	}
	Switch ($server){
	...
	Param(
		...
		Try {
			...
			# 11.01.2018 JJK: Using displayname is comprehensive and the start does work against them recursively
			$priInstance = get-scomclassinstance -Class $scomClass | Where-Object {$_.DisplayName -eq $serverObject.DisplayName}
	...
	# Moving to the start and stop functions
	# $procStart = [System.Diagnostics.StopWatch]::StartNew()
...
END {
	...
	Remove-Variable -Name server,omServer, MMTime, MMEnd, MMReason, MMComment, allinstances, priInstance, curMaintMode, agentObjects -ErrorAction SilentlyContinue
	Write-Verbose -Message "[END]End remote session"
	Remove-Item Function:Start-BAOMaintenanceMode
	Remove-Item Function:Stop-BAOMaintenanceMode
