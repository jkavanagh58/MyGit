...
.NOTES
	...
	08.23.2018 JJK:	Restored FQDN for Wintel version
#>
[CmdletBinding()]
Param(
...
Begin {
	...
	Try {
		...
		Invoke-Command -Session $bypass -ScriptBlock {get-scomagent -Name $using:serverFQDN -ErrorAction Stop} | out-null
	}
	Catch {
...
Process {
	...
	Invoke-Command -session $bypass -ScriptBlock {
		...
		ForEach ($agentObject in $agentObjects) {
			# Look at implementing a switch based on object type and current Maintenance Mode state
