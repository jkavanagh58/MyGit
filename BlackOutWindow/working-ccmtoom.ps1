...
Begin {
	...
	$OpsMgr = New-Pssession -computername "WFM-OM12-MSM-01.wfm.wegmans.com" -Name OpsMgr -Credential $admcreds
	...
	Invoke-command -Session $OpsMgr -ScriptBlock {Start-OperationsManagerClientShell -ManagementServerName: "" -PersistConnection: $true -Interactive: $true}
...
Process {
	...
	ForEach ($ccmColl in $ccmcollections){
		...
		# Execute scriptblock
		$sb | out-file c:\etc\scomrun.txt -Append
