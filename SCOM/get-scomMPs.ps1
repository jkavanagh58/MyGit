...
function Get-MSDownloadFile{ 
    ...
    { 
        ...
        if (Test-Path -path "$Path\$Filename") { 
            ...
            $webClient = Invoke-WebRequest -Uri $URI -OutFile $Path\$FileName -PassThru -Proxy "http://ohhproxy01:8080"
...
#>
    ...
    { 
        ...
        $allmpspage = Invoke-WebRequest -Uri "http://social.technet.microsoft.com/wiki/contents/articles/16174.microsoft-management-packs.aspx" -Proxy "http://ohhproxy01:8080" -ProxyUseDefaultCredentials 
...
# Make it happen 
Get-SCOMManagementPacks -MPPath D:\MPs -CMTrace | ? {$_.status -notlike "Unchanged"} | sort status 